...
PKG_DIR = ROOT_DIR / "libs" / "llmp" / "llmp"
...
def _load_package_modules(
        package_directory: Union[str, Path], submodule: Optional[str] = None, subsubmodule: Optional[str] = None
    ...
    if subsubmodule is not None:
        package_path = package_path / subsubmodule
...
def _document_llmp() -> None:
    """Document the main package."""
    # load top level module members
    llmp_members = _load_package_modules(PKG_DIR)
    ...
    llmp_members.update(
        {
            ...
            "components.optimizer": components["optimizer"],
            "components.example_manager": components["example_manager"],
        }
    )

    lc_doc = ".. _api_reference:\n\n" + _construct_doc("llmp", llmp_members)
